...
const OrdenProduccionCard: React.FC<OrdenProduccionCardProps> = ({ ordenProduccion }) => {
    ...
    const pdfGenerator = useMemo(() => new ODPpdfGenerator(), []);

    const handleDownloadPdf = useCallback(async () => {
        try {
            await pdfGenerator.downloadPDF(ordenProduccion);
        } catch (error) {
            console.error("Error generating production order PDF", error);
            toast({
                status: "error",
                title: "Error al generar PDF",
                description: "No se pudo descargar el PDF de la orden de producción.",
            });
        }
    }, [ordenProduccion, pdfGenerator, toast]);
        ...
        <Box borderWidth="1px" borderRadius="lg" p={4} mb={4} boxShadow="sm">
            <Flex direction="column">
                ...
                >
                    ...
                    <HStack alignSelf={{ base: 'flex-start', md: 'center' }} spacing={2}>
                        <Badge colorScheme={estadoOrdenInfo.colorScheme}>{estadoOrdenInfo.label}</Badge>
                        <Menu>
                            <MenuButton
                                as={IconButton}
                                aria-label="Opciones de orden de producción"
                                icon={<FiMoreVertical />}
                                variant="ghost"
                                size="sm"
                            />
